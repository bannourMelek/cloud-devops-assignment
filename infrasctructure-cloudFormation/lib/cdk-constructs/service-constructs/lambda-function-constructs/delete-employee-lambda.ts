...
export class DeleteEmployeeLambda extends LambdaFunctionBase {
  constructor(stack: ServiceStack, props: MicroserviceLambdaProps) {
    super(stack, `${APP_NAME}${DeleteEmployeeLambda.name}`, {
      ...
      handler: getPythonFunctionHandlerName(DeleteEmployeeLambda),
      functionName: getFunctionName(DeleteEmployeeLambda),
      memorySize: 128,
      environment: {
        DYNAMODB_TABLE:
